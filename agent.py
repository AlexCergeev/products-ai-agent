...
code_finder = Agent(
    role_description=('''Ты опытный программист. Тебе на вход подается большой файл, содержащий обычный текст и программный код. 
Твоя задача — удалить из файла весь обычный текст и все комментарии, оставив только код. 
Не изменяй сам код. Выведи итоговый результат.'''),
    model=gigachat_model,
    memory=shared_memory,
    name="Код"
)
...
3. Если найдены несоответствия или ошибки, опиши их по пунктам.
...
2. Выдели все обнаруженные ошибки:
...
def main_workflow():
    ...
    with open('/Users/kpaq/Documents/code/products-ai-agent/test_er1_in_balance.txt', 'r') as file:
    ...
    code_llm = code_analyst.run("", memory_key_read='Требования пользователя')
